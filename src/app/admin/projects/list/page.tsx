...
export default function ProjectsPage() {
  ...
  }, [fetchAllProjects, currentPage, pageSize, setPageSize, selectedCategory]);
  ...
  async function handleDeleteProject(getId: string) {
    if (window.confirm("Are you sure you want to delete this project?")) {
      const result = await deleteProject(getId);
      if (!result) {
  ...
  async function handleEditRoute(getId: string) {
    console.log('getId', getId);
    router.push(`/admin/projects/edit/${getId}`)
  }
  return (
    <main
      id="projects-root"
    ...
    >
      <div className="flex justify-between items-center cursor-pointer">
        ...
        <Link href={'/admin/projects/add'}>
          <Button className="flex items-center justify-center p-4 rounded-md cursor-pointer">Add project</Button>
        </Link>
        ...
        <div className="flex flex-wrap gap-3">
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-250 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-400 ${selectedCategory === cat
                ...
                >
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex space-x-2">
                      <Pen size={16} className="text-blue-600 hover:text-blue-900 cursor-pointer" onClick={() => handleEditRoute(project.id)} />
                      <Trash size={16} className="text-red-500 hover:text-red-700 cursor-pointer" onClick={() => handleDeleteProject(project.id)} />
                    </div>
                  </div>
                </motion.li>
              ))}
